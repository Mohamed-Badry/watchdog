...
print(
    f"Simulating passes for {len(my_sats)} satellites over next {HOURS_AHEAD} hours..."
)
...
for sat in my_sats:
    ...
    for ti, event in zip(times, events):
        if event == 0:  # Rise
            ...
            if current_rise is not None:
                ...
                mid_time = ts.from_datetime(
                    start_dt + datetime.timedelta(minutes=duration / 2)
                )
    ...
    if pass_count > 0:
        name = candidates[candidates["norad_cat_id"] == sat.model.satnum].iloc[0][
            "amsat_name"
        ]
...
print(
    df_scores[["name", "pass_count", "total_duration", "max_el"]].to_string(index=False)
)
...
legend_elements = [
    Line2D([0], [0], color=color_map[n], lw=2, label=n) for n in unique_names
]
ax.legend(handles=legend_elements, loc="upper right", bbox_to_anchor=(1.3, 1.0))

plt.title(
    f"Skyplot: Passes of Top {TOP_N_SATS} Satellites\n(Next {HOURS_AHEAD}h, >30° El)",
    pad=20,
    fontsize=14,
    fontweight="bold",
)
