...
def dict_to_html(data, title="Data"):
    ...
    """
    if not data:
        return "<div style='color: #888; font-style: italic;'>Empty</div>"

    html_out = "<div style='font-family: monospace;'>"
    ...
    for k, v in data.items():
        ...
        if isinstance(v, dict):
            # Nested
            html_out += "<div style='margin-bottom: 4px;'>"
            html_out += f"<span style='{key_style}'>{k}</span>:"
            html_out += "<div style='margin-left: 15px; border-left: 2px solid #eee; padding-left: 5px;'>"
            ...
            if "volt" in k_lower:
                unit = " V"
            elif "curr" in k_lower:
                unit = " A"
            elif "temp" in k_lower:
                unit = " °C"
            elif "rssi" in k_lower:
                unit = " dBm"
...
class TelemetryInspector:
    def __init__(self):
        ...
        self.day_dropdown = widgets.Dropdown(
            options=[], description="Day:", layout=widgets.Layout(width="200px")
        )
    ...
    def _load_days(self, norad_id):
        ...
        try:
            self.day_dropdown.unobserve(self._on_day_change, names="value")
        except ValueError:
            pass
    ...
    def _render_frame(self, norad_id, idx, frame):
        ...
        self.header_label.value = (
            f"<h3>Frame #{idx} | {timestamp} | NORAD {norad_id}</h3>"
        )
        ...
        self.panel_struct.value = (
            f"<h4>Binary Struct (Construct)</h4>{dict_to_html(struct_data)}"
        )
        ...
        try:
            ...
            telemetry_dict = (
                telem.to_dict()
                if telem
                else {"status": "Decoder Failed (CRC/Validation)"}
            )
        except Exception as e:
            telemetry_dict = {"error": str(e)}

        self.panel_telem.value = (
            f"<h4>Standardized (Golden Features)</h4>{dict_to_html(telemetry_dict)}"
        )
